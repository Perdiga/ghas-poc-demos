...
etag=$(curl -s -H "Authorization: token $GITHUB_PAT" \
    ...
    https://api.github.com/repos/tweag/ghas-poc-demos/code-scanning/alerts?severity=high -D - | grep ETag)

# Make the actual request to fetch the data
response=$(curl -s -H "Authorization: token $GITHUB_PAT" \
    -H "Accept: application/vnd.github+json" \
    -H "If-None-Match: \"$etag\"" \
    https://api.github.com/repos/tweag/ghas-poc-demos/code-scanning/alerts?severity=high)

# Check if the response is valid JSON before passing it to jq
if echo "$response" | jq empty > /dev/null 2>&1; then
    echo "High severity alerts received: "
    echo "$response" | jq .
else
